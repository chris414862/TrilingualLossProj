...
def compute_spectrogram(y, native_sample_rate, audio_conf={}, trunc_cnt:int=None):
    ...
    # auto_length will set target_length such that X proportion of the data is left un-truncated
    ...
    if p > 0:
        ...
        trunc_cnt = None if trunc_cnt is None else trunc_cnt + 1
        trunc_cnt_str = "" if trunc_cnt is None else str(trunc_cnt)+" image(s) have"
        print(f'WARNING: {trunc_cnt_str} exceeded target length of {target_length}.', end=" ")
        print('Truncated %d frames out of %d total' % (-p, n_frames))
        logspec = logspec[:,0:p]
        n_frames = target_length

    logspec = torch.FloatTensor(logspec)
    
    if trunc_cnt is None:
        return logspec, n_frames
    else:
        return logspec, n_frames, trunc_cnt
