...
def split_target_online_views(model_outputs, view_ids, args):
    target_output = model_outputs.pop(args.byol_target_view)
    target_id = args.target_view
    online_outputs = model_outputs #target removed with pop
...
def byol_computation(image_model, image_input, audio_models:dict, target_audio_input:dict, device, args, 
                     aux_nets=None, **extra_loss_kwargs):
    ...
    for online_id in online_ids:
        online_projs[online_id] = aux_nets[online_id]["proj"](online_reps[online_id])

    ### get predictions (online only) ###
    online_preds = {}
    for online_id in online_ids:
        online_preds[online_id] = aux_nets[online_id]["pred"](online_projs[online_id])
    ...
    loss_record = {}
    for online_id in online_ids:
        loss = mse(target_proj, online_preds[online_id])
